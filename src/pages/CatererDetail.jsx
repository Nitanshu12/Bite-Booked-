...
export default function CatererDetail() {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="relative w-full min-h-[340px] md:h-80 lg:h-96 overflow-hidden">
        <img src="/images/hero-bg.png" alt="background" className="w-full h-full object-cover opacity-60" />
        <div className="absolute inset-0 bg-black/60" />
        <div className="absolute inset-0 flex flex-col md:flex-row items-center md:items-end justify-center md:justify-start gap-4 md:gap-8 px-4 md:px-16 pb-4 md:pb-12">
          <img src={caterer.logo} alt={caterer.name} className="w-32 h-32 sm:w-40 sm:h-40 md:w-56 md:h-56 rounded-lg object-contain bg-white p-4 shadow-xl border-4 border-white mb-4 md:mb-0" />
          <div className="flex-1 flex flex-col items-center md:items-start justify-center md:justify-end text-center md:text-left">
            <div className="flex flex-col md:flex-row items-center md:items-end gap-2 mb-2 mt-0">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mr-0 md:mr-2">{caterer.name}</h1>
            ...
            <div className="text-gray-200 text-sm sm:text-base md:text-lg mb-2">
              <span className="font-bold text-white">Location:</span> CaterNinja GravyTech Catering Ventures Pvt Ltd. NH-23, New Delhi-110045
            </div>
            <button className="mt-2 bg-orange-500 hover:bg-orange-600 text-white font-bold px-8 py-3 rounded-lg shadow transition text-base sm:text-lg w-full max-w-xs mx-auto md:w-fit md:self-start md:mx-0 cursor-pointer mb-4" onClick={handleBookBite}>BOOK BITE</button>
          </div>
        </div>
      </div>
